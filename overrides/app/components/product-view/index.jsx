    ...
    (
        {
            ...
            imageSize = 'md', // Reserved for future use or backward compatibility
        ...
        // Extract images from imageGroups for ProductImageGallery
        const productImages = useMemo(() => {
            if (!product?.imageGroups || product.imageGroups.length === 0) {
                return []
            }

            // Find the large image group for the current variation
            const largeImageGroup = findImageGroupBy(product.imageGroups, {
                viewType: 'large',
                selectedVariationAttributes: variationParams
            })

            // Return images array from the found group, or empty array
            return largeImageGroup?.images || []
        }, [product?.imageGroups, variationParams])
        ...
        return (
            <Flex direction={'column'} data-testid="product-view" ref={ref}>
                ...
                <Flex direction={['column', 'column', 'column', 'row']} align="flex-start">
                    {showImageGallery && (
                        <Box
